...
2. [Zone Map and Rules](#section-2--zone-map-and-rules)
3. [Vehicle Lifecycle](#section-3--vehicle-lifecycle)
4. [Lot Placement Rules](#section-4--lot-placement-rules)
5. [Signage Standards](#section-5--signage-standards)
6. [Key Cafe Protocol](#section-6--key-cafe-protocol)
7. [PDI Compliance](#section-7--pdi-compliance)
8. [Daily Operations Overview](#section-8--daily-operations-overview)
9. [Escalation Protocol](#section-9--escalation-protocol)
10. [Financial Penalties](#section-10--financial-penalties)
11. [Cross-Reference Index](#section-11--cross-reference-index)
...
**Purpose:** Primary customer-facing display area. The front-line showroom.
...
> ⚠️ NEVER: Place any of the following vehicles in the Cage:
...
> ⚠️ NEVER: Place these in the East Side Fence Line:
...
> ⚠️ NEVER: Place these in the West Side of Building:
> - NEW vehicles — new arrivals go to Cage (after PDI/detail), not here
...
**Allowed vehicles:** Power sport deliveries, Boat deliveries, Hysen units (seasonal — after snow melts)

> ⚠️ NEVER: Place retail vehicles of any kind in the Power Sport / Quad Corner. Any vehicle requiring door access for customers or staff to enter is prohibited here.
...
→ Decision Tree: staff-parking-new-employee.md (first violation — new employee)
→ Decision Tree: staff-parking-repeat-violation.md (repeat violation)
...
| Pending PDI | New vehicle arrived, awaiting Pre-Delivery Inspection. | `AVAILABLE` (or as explicitly set) — **NEVER `IN RECON`** | Awaiting service routing |
| PDI In Progress | Being actively inspected by a service technician. | Not tracked separately in Airtable | Service department |
...
| Front-Line Ready (FLR) | Fully PDI'd + fully detailed + no stickers/tape + stock-in tag placed. | `AVAILABLE` or `DEMO` in Airtable | Cage |
| On Display | Vehicle physically in the Cage. | `AVAILABLE` or `DEMO` | Cage |
| Sold | Customer has purchased the vehicle. | `SIGNED DEAL` or `WHOLESALE \| SOLD` | East Side Fence Line |
| Booked Not Delivered (BND) | Deal is active on paper but vehicle not yet handed over. | `BOOKED \| NOT DELIVERED` | East Side Fence Line (or West Side overflow) |
| Trade-In | Vehicle received from a customer as part of a deal. | Varies by routing decision | Varies by condition |
| Customer Vehicle | A customer's personal vehicle on the lot. Does NOT belong to the dealership. | Not an inventory item — must be signed | Designated area; signed |
| Recon | Vehicle undergoing reconditioning or mechanical work. | `IN RECON`, `INCOMING`, `WHOLESALE`, or `CHASE` | West Side of Building |
| Dealer Trade Incoming | Vehicle arriving from another dealership. Requires final PDI + full detail before placement. | `AVAILABLE` (applied on arrival) | Awaiting service routing |
| Non-Prime / Division One | Vehicle identified as Non-Prime division. **Prohibited on this lot.** | Airtable `STOCK HOLDER` = `NON PRIME DIVISION` | Must be removed immediately |
...
> ⚠️ NEVER: Give a new arrival "Recon" status. A new vehicle that arrives and needs PDI is "Pending PDI" — NOT "RECON." Classifying a new arrival as RECON hides it from the sales inventory system. Sales staff cannot see it, cannot sell it, and it generates zero revenue while occupying a slot.

> ⚠️ NEVER: Skip or reorder the ship mode clearance sequence. A vehicle in ship mode must have: (1) battery reconnected AND (2) software reset performed AND (3) PDI completed — BEFORE it can be placed on the front line OR delivered to a customer. No exceptions. → Decision Tree: vehicle-arrival-ship-mode.md

> ⚠️ NEVER: Skip the PDI for dealer trades. Even if the originating dealership says the vehicle is already inspected and ready, the Edmonton Office must perform its own final PDI. → Decision Tree: vehicle-arrival-dealer-trade.md

> ⚠️ NEVER: Allow a non-prime vehicle to remain on this lot for any reason. Any vehicle with Airtable `STOCK HOLDER` = `NON PRIME DIVISION` must be routed to AB | STURGEON DODGE immediately. → Decision Tree: vehicle-non-prime-identification.md
...
→ Decision Tree: vehicle-non-prime-identification.md
...
→ Decision Tree: vehicle-categorization.md
...
**Color:** **White. Do NOT use yellow.** Yellow tags look like AutoWorld tags and cause identification confusion.
...
**Enforcement when missing:** → Decision Tree: sold-sign-missing-enforcement.md (3-strike escalation protocol)
...
Every vehicle without a sign creates the same risk. **This will not happen again.**
...
**There is no grace period.** The agreement must be signed before any access is granted — on Day 1, before any key or plate is touched.

→ Decision Tree: accountability-agreement-onboarding.md
...
> ⚠️ NEVER: Pass a key or dealer plate directly to another employee — under any circumstances, for any reason. If another employee asks you for a plate or key you currently have signed out, the response is always:
...
Every set of keys at the Edmonton Office has a GPS tag attached.
...
→ Decision Tree: key-plate-lost-response.md
...
→ Decision Tree: vehicle-arrival-ship-mode.md

> ⚠️ NEVER: Deliver a vehicle in ship mode to a customer. Confirmed incident at this dealership: A vehicle was delivered to a customer while still in ship mode. The battery was disconnected. The car died the next day. The vehicle had to be towed back to the dealership. Total cost: **$275** (skipped detail rework) + tow cost + complete rework of the entire preparation process. The customer was stranded.
...
→ Decision Tree: vehicle-arrival-dealer-trade.md
...
→ Decision Tree: vehicle-pdi-routing.md
→ Decision Tree: vehicle-pdi-compliance-deadline.md
...
→ Decision Tree: vehicle-recon-routing.md
...
| Sold sign missing | Sales Manager (Kevin) | 3-strike escalation — → Decision Tree: sold-sign-missing-enforcement.md |
...
| Staff vehicle on lot | Lot Manager / Management | Address with employee — → Decision Tree: staff-parking-repeat-violation.md |
| PDI deadline at risk | Service Department Lead | Alert with vehicle info and deadline date |
| Unknown vehicle | Lot Manager | → Decision Tree: vehicle-status-unknown.md |
...
| Situation | Action |
|---|---|
| Staff vehicle parked on lot | Address immediately — → Decision Tree: staff-parking-repeat-violation.md |
...
| Sold vehicle without sign | 3-strike escalation — → Decision Tree: sold-sign-missing-enforcement.md |
...
All decision trees are located in: `systems-analysis/decision-trees/`
